...
import { deployment, ethers, network } from "hardhat";
...
async function main() {
  ...
  const tableState = await TableState.deploy(
    ...
    ethers.toUtf8Bytes(deployment.jobId) // Must convert the string to `bytes32`
  );
  await tableState.deploymentTransaction();
  console.log(`Contract 'TableState' deployed to: ${await tableState.getAddress()}`);
  ...
  const contractAddr = await tableState.getAddress();
  ...
  const amount = ethers.parseUnits("3.0");
